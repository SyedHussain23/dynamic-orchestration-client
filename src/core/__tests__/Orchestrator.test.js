...
function makeNodes(seeds, faultProbability = 0) {
    const ids = ['node-a', 'node-b', 'node-c'];
    return ids.map((id, i) => new SimulatedStreamNode({
        id,
        corpus: CORPUS,
        meanChunkMs: 1,
        jitterMs: 0,
...
async function runAndCapture(o) {
    const cap = {
        chunks: [],
        events: [],
        nodesUsed: new Set(),
...
function capturingOrchestrator(nodes, cfg = {}) {
    const cap = {
        chunks: [],
        events: [],
        nodesUsed: new Set(),
        ...
        const flaky = {
            ...
            async *stream({ resumeFrom, signal }) {
                for (let i = resumeFrom; i < CORPUS.length; i++) {
                    if (signal.aborted)
                        throw new NodeError('node-a', 'abort');
                    await new Promise((r) => setTimeout(r, 1));
                    if (chunksBeforeDrop >= 5)
                        throw new NodeError('node-a', 'drop');
                    chunksBeforeDrop++;
