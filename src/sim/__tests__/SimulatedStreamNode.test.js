...
describe('SimulatedStreamNode', () => {
    ...
    it('rejects invalid resumeFrom values', async () => {
        ...
        const ac = new AbortController();
        await expect(collect(n.stream({ resumeFrom: -1, signal: ac.signal }))).rejects.toThrowError(RangeError);
        await expect(collect(n.stream({ resumeFrom: CORPUS.length + 1, signal: ac.signal }))).rejects.toThrowError(RangeError);
    });
